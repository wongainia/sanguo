 ...
 * 
 */
public class SecurePayHelperUnionpay {
	static final String TAG = "MobileSecurePayHelper";
	
	private ProgressDialog mProgress = null;
	Context mContext = null;

	public SecurePayHelperUnionpay(Context context) {
	...
	public boolean detectMobile_sp() {
		...
		if (!isMobile_spExist) {
			...
			//
			// 捆绑安装
			retrieveApkFromAssets(mContext, "UnionPayPlugin.apk",
					cachePath);

			mProgress = BaseHelper.showProgress(mContext, null, "正在检测银联安全支付服务版本",
					false, true);
			...
			new Thread(new Runnable() {
				public void run() {
					...
					msg.what = AlipayDefine.RQF_INSTALL_CHECK;
					msg.obj = cachePath;
	...
	public void showInstallConfirmDialog(final Context context,
		...
		tDialog.setMessage("为保证您的交易安全,需要您安装银联安全支付服务插件才能进行付款.\n\n点击确定,立即安装.");
	...
	public boolean isMobile_spExist() {
		...
		for (int i = 0; i < pkgList.size(); i++) {
			...
			if (pI.packageName.equalsIgnoreCase("com.tct.union.pay"))
				return true;
		}
	...
	public JSONObject sendCheckNewUpdate(String versionName) {
		...
		try {
			...
			req.put(AlipayDefine.action, AlipayDefine.actionUpdate);

			JSONObject data = new JSONObject();
			data.put(AlipayDefine.platform, "android");
			data.put(AlipayDefine.VERSION, versionName);
			data.put(AlipayDefine.partner, "");

			req.put(AlipayDefine.data, data);
	...
	public JSONObject sendRequest(final String content) {
		...
		try {
			...
			synchronized (nM) {
				//
				response = nM.SendAndWaitResponse(content, AlipayDefine.server_url);
			}
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
			try {
				switch (msg.what) {
				case AlipayDefine.RQF_INSTALL_CHECK: {
					//
					closeProgress();
