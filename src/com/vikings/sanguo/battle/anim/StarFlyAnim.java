...
public class StarFlyAnim extends FrameLayout implements OnGlobalLayoutListener {
	...
	public StarFlyAnim(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init();
	}

	public StarFlyAnim(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	private void disapper() {
		int size_init = getChildCount();
		if(size_init > 20)
		{
			for(int index = 0; index < size_init-20; index ++)
			{
		...
		for (int i = size - 1; i >= 0; i--) {
			final ImageView txt = (ImageView) getChildAt(i);
			...
			//final int index = i;
			//Log.e("StarFlyAnim", "start anim  index:"+index+ "animation:" +txt );
			animSet.setAnimationListener(new AnimationListener() {
				...
				public void onAnimationEnd(Animation animation) {
					//Log.e("StarFlyAnim", "end anim index:"+index+ "animation:" +txt );
					txt.clearAnimation();
	...
	private boolean show() {
		if (width > 0 && height > 0 && vecKeywords != null
			...
			for (int i = 0; i < size; i++) {
				...
				final ImageView txt = new ImageView(getContext());

				int strWidth = Config.getController()
						.getDrawable(R.drawable.effect_star)
						.getIntrinsicWidth();
	...
	private void attach2Screen(LinkedList<ImageView> listTxt, int xCenter,
		...
		for (int i = 0; i < size; i++) {
			...
			Drawable d = Config.getController().getDrawable("effect_star", 45,
					45);
	...
	public AnimationSet getAnimationSet(int[] xy, int xCenter, int yCenter,
		...
		if (type == OUTSIDE_TO_LOCATION) {
			...
			TranslateAnimation translate = new TranslateAnimation(0,
					((xy[IDX_X] + (xy[IDX_TXT_LENGTH] >> 1) - xCenter) << 1),
					0, ((xy[IDX_Y] - yCenter) << 1));

			float dx = (xy[IDX_X] + (xy[IDX_TXT_LENGTH] >> 1) - xCenter) << 1;
			float dy = (xy[IDX_Y] - yCenter) << 1;
			animSet.addAnimation(translate);
		...
		// int startOffset = 0;
		// startOffset = Math.abs(new Random().nextInt()%300);
