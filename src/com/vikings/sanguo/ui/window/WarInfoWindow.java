...
public class WarInfoWindow extends CustomPopupWindow {
	...
	private boolean isClickBattle = false;// 防止重复点击导致，提示围城结束
			...
			l = new OnClickListener() {
				...
				public void onClick(View v) {
					if(bfic != null && bfic.isHoly())
					{
						...
						try {
							hpHolyProp = (HolyProp) CacheMgr.holyPropCache.get(bfic
									.getId());
						...
						// 如果自己援助自己不需要消耗物品
						if (isGoShop) {
							if (!bfic.isMyFief() && rbic != null
									&& !rbic.isAttacker() && !rbic.isDefender()) {
								if (!FiefDetailTopInfo.SpecialConsumableIsEnough(
										hpHolyProp, Constants.ASSISTATTACK)) {
	...
	private void battleConfirm() {
		if (isClickBattle) {// 防止重复点击导致，提示围城结束
	...
	private boolean reinforceData() {
		...
		if (!bfic.isMyFief() && bfic.getAttackerId() == Account.user.getId()) {
			isReinAttack = true;
		} else if (bfic.isMyFief()
				&& bfic.getAttackerId() != Account.user.getId()) // 自己的领地 协助防守自己
		{
			isReinAttack = false;
		} else {
			if (rbic.isAttackerById(bfic.getAttackerId())) {
				if ((bfic.isHoly() && rbic.canReinforceAttackInShenJi())
						|| (!bfic.isHoly() && rbic.canReinforceAttack())) {
					isReinAttack = true;
				}
			} else {
				if ((bfic.isHoly() && rbic.canReinforceDefendInShenJi())
						|| (!bfic.isHoly() && rbic.canReinforceDefend())) {
					isReinAttack = false;
				}
			}
		}
